...
obsolete
    1)to merge several mc16 productions :
    MODE_MERGE_yields_subprocesses=1
    put the relevant sample to merge in : vec_string_sample
    put the relevant sample of destination: string_sample_for_merging_subprocesses
    check if case is considered for MODE_MERGE_yileds_subprocesses

    2)read the results
    MODE_MERGE_yields_subprocesses=0
    put the relevant sample of destination: vec_string_sample
