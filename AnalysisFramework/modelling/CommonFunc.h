...
namespace CommonFunc{
  ...
  TH1D* RerangeTH1D(TH1D *hold, double xmin_new, double xmax_new, TString tag="");
  int sign(double x);
...
/*   bool descending(int i, int j); */
  ...
  void AddGraphs(TGraph* gr1, TGraph* gr2);
  void AddGraphsError(TGraphErrors* gr1, TGraphErrors* gr2);
  TGraphErrors* HistoToGraph(TH1D* h);
  TGraph* scaledGraph(TGraph* g, double w);
  double getChi2(TH1D* h, TGraph* gr);
