...
void VariablePlotter::execute()
{
  ...
  for (auto iSample:document.samples.samples) {
    ...
    for (auto iCut: cutFlows){
      ...
      std::vector<std::string> truthRecs = {""};        
      // Temporarily disable the line below, until I've really fixed it
      //if (sampleName == "yy") truthRecs = {" && HGamAntiKt4EMTopoJetsAuxDyn.HadronConeExclTruthLabelID[0]==5"," && HGamAntiKt4EMTopoJetsAuxDyn.HadronConeExclTruthLabelID[0]==4", " && (HGamAntiKt4EMTopoJetsAuxDyn.HadronConeExclTruthLabelID[0]!=4 || HGamAntiKt4EMTopoJetsAuxDyn.HadronConeExclTruthLabelID[0]!=5)"};
      for (auto rec : truthRecs) {
        for (auto iMC: mcCampaigns){
          ...
          for(auto iVar : document.variables.varMap){
            ...
            select = select.std::string::replace(select.find("HGamEventInfoAuxDyn.isPassed"), std::string("HGamEventInfoAuxDyn.isPassed").length(), "HGamEventInfoAuxDyn.isPassed"+rec);
        ...
        // Make sensible names for the additional truht requirements
        std::string recName = "";
        if (rec == " && HGamAntiKt4EMTopoJetsAuxDyn.HadronConeExclTruthLabelID[0]==4") recName = "cj";
        if (rec == " && HGamAntiKt4EMTopoJetsAuxDyn.HadronConeExclTruthLabelID[0]==5") recName = "bj"; 
        // Write to the plotting directory 
        DIR* dir = opendir("plots");
        if (!dir) const int dir_err = mkdir("plots", S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH);                   
        TFile outfile (("plots/"+sampleName+recName+"_"+iCut+".root").c_str(),"recreate");
