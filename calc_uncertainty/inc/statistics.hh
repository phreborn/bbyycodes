...
public:
  ...
  static void calcBandInterval(RooCurve *curCurve, vector<RooCurve*> variations,Int_t i,Double_t Z, Double_t& lo, Double_t& hi, Bool_t approxGauss);
  static RooCurve* makeErrorBand(RooCurve *curCurve, vector<RooCurve*> variations, Double_t Z);
  static RooCurve* convertAsimovToCurve(RooDataSet *asimov, TString observableName, bool isHist, double epsilon=1e-6);
  static void releaseTheGhost(RooDataSet *obsdata, RooRealVar *x, RooRealVar *w, double ghostwt=1e-9);
  static RooCurve* createBand(RooCurve *cenCurve, RooAbsPdf *pdf, RooRealVar *x, RooFitResult *res, RooArgSet *params, double Z=1, int scale=1, bool isHist=false);
  static RooFitResult* profileToData(ModelConfig *mc, RooAbsData *data, TString _minimizerAlgo="Minuit2", int _minimizerStrategy=1, double _minimizerTolerance=1e-5, int _printLevel=1, bool doHesse=false, bool doMinos=false);
  static void fixSyst(ModelConfig *mc, bool fix=true, bool reset=false);
