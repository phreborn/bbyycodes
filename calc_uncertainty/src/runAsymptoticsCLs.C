		       ...
		       TString option)
{
  ...
  runAsymptoticsCLs(infile, workspaceName, modelConfigName, dataName, asimovDataName, folder, smass.str(), CL, option);
}
...
//check inputs
  ...
  if(option.Contains("starthigher")) initial_guess_obs*=100;
...
int main(int argc, char* argv[]){
  ...
  if(option.Contains("histfactory")){
    wname="combined";
    mname="ModelConfig";
    dname="obsData";
  }

  if(option.Contains("simone")){
    wname="ws_merged";
    mname="ModelConfig";
    dname="data";
  }
  ...
  if(option.Contains("roostatsasimov")) useRooStatsAsimov=true;

  if(option.Contains("higheps")){
    defaultPrecision=0.0001;
  }
  if(option.Contains("strat1")) defaultStrategy=1;
  if(option.Contains("strat2")) defaultStrategy=2;
  
  if(option.Contains("starthigh")) defaultInitGuess=10;
  if(option.Contains("starthigher")){
    cout<<"REGTEST: starting from 100 as initial guess"<<endl;
    defaultInitGuess=100;
  }
  if(option.Contains("startlow")){
    cout<<"REGTEST: starting from 0.1 as initial guess"<<endl;
    defaultInitGuess=0.1;
  }
  if(option.Contains("forceasimov")){
    aname="asimovData_0_do_it_aaron";
  }
  if(option.Contains("debug")) verbose=true;
  double CL=0.95;
  if(m_highCL){
