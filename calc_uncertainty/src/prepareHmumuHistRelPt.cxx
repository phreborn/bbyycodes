...
int main(int argc, char** argv){
  ...
  if(argc<5){
    printf("Usage %s <input> <output> <coef 1> <coef 2>");
    exit(0);
  }
  ...
  for(long int ievt=0;ievt<nevt;ievt++){
    ...
    bool old_analysis_cut = (fabs(p->Muons_Eta_Lead) <= 2.5 && fabs(p->Muons_Eta_Sub) <= 2.5);
    if(!ss_cut || !old_analysis_cut) continue;
    bool relpt_cut = (p->Muons_PT_Lead>p->Muons_Minv_MuMu*coef1) && (p->Muons_PT_Sub>p->Muons_Minv_MuMu*coef2);
    if(!relpt_cut) continue;
