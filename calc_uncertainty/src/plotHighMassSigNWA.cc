...
int FileNameToCat(TString fileName){
  ...
  if(fileName.Contains("H750")) return 0;
  return -1;
}
...
int main(int argc, char **argv){
  ...
  for(int ipoint=0;ipoint<npoint;ipoint++){
    widthArr[ipoint]=0.004;
    xMinArr[ipoint]=massArr[ipoint]-100;
    xMaxArr[ipoint]=massArr[ipoint]+100;
