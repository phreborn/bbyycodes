...
int main(int argc, char **argv){
  ...
  if(Opt["1D"]){
    ...
    hframe->GetXaxis()->SetTitle(title[poiName]);
    ...
    TLegend *leg=FastLegend(x2-0.3,y2-0.25,x2-0.03,y2-0.05,0.04);
	...
	TString inputScanFile="root/"+inputSet[isam]+Form("/*%s=%.2f_.*.root",poiName.Data(),poiValue);
	...
	cout<<nll[i]<<" "<<mH[i]<<endl;
      ...
      gr->GetXaxis()->SetTitle(title[poiName]);
    ...
    if(option.Contains("2016")) pavetext.push_back("Data 2016, #sqrt{#it{s}} = 13 TeV, 33.9 fb^{-1}");
  ...
  if(Opt["2D"]){
    ...
    hframe->GetXaxis()->SetTitle(title[poiXName]);
    hframe->GetXaxis()->SetRangeUser(xmin,xmax);
