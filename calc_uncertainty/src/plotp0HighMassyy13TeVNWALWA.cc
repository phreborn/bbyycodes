...
pair<double, double> readInput(TString inputDir, double array[npoints]){
  ...
  for(int i=0;i<npoints;i++){
    ...
    TString inputFilefix=inputDir+Form("/mX_%.2f*wX*_mufix.root",mass);
    TString inputFilehat=inputDir+Form("/mX_%.2f*wX*_muhat.root",mass);

    double nllfix, nllhat, muhat;
    int statusfix, statushat;
    unique_ptr<TChain> cfix(new TChain("toys/nllscan"));
    if(!cfix->Add(inputFilefix)){
    ...
    if(!isfinite(pvalue)||statusfix!=0||statushat!=0){
...
int main(int argc, char** argv) 
{
  ...
  pair<double, double> maxdata=readInput("pvalue/high_mass_diphoton_13TeV_2D_lwapvalue/",data);
  double Z = maxdata.second;
  double M_minimum = maxdata.first;

  pair<double, double> maxdata2=readInput("pvalue/high_mass_diphoton_13TeV_2D_nwapvalue/",data2);
  ...
  g_data->SetMaximum(5e1);
  g_data->SetMinimum(1e-5);
  //g_2->Draw("ae3");
  //g_1->Draw("e3");
  ...
  leg->AddEntry(g_data,"LWA","L");
  leg->AddEntry(g_data2,"NWA","L");
  ...
  PrintCanvas(c1,"fig/plot_p0/high_mass_diphoton/pvalue_NWA_LWA_13TeV");
