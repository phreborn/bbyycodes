...
int main(int argc, char** argv) 
{
  ...
  pair<double, double> maxdata=readInput("pvalue/combined_VV_HVT_WZ_20160126",data);
  double Z = maxdata.second;
  double M_minimum = maxdata.first;

  pair<double, double> maxdata2=readInput("pvalue/combined_VV_HVT_WW_20160126",data2);
  double Z2 = maxdata2.second;
  double M2_minimum = maxdata2.first;

  pair<double, double> maxdata3=readInput("pvalue/combined_VV_HVT_comb_20160126",data3);
  ...
  g_data->GetXaxis()->SetTitle("m_{HVT} [GeV]");
  ...
  g_data3->SetLineColor(1);
  g_data3->SetLineStyle(1);
  g_data3 -> Draw("l");

  TLegend *leg=FastLegend(0.20,0.76,0.45,0.93,0.05);

  leg->AddEntry(g_data,"HVT#rightarrowWZ","L");
  leg->AddEntry(g_data2,"HVT#rightarrowWW","L");
  leg->AddEntry(g_data3,"HVT#rightarrowWZ/WW","L");
  ...
  system("mkdir -vp fig/plot_p0/combined_VV_HVT/");
  PrintCanvas(c1,"fig/plot_p0/combined_VV_HVT/pvalue");
