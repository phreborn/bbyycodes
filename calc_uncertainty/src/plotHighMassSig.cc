...
int FileNameToCat(TString fileName){
  ...
  if(fileName.Contains("H750")) return 1;
  if(fileName.Contains("H800")) return 2;
  if(fileName.Contains("H1500")) return 3;
  return -1;
}
...
int main(int argc, char **argv){
  if(argc<5){
    cout<<"Usage: "<<argv[0]<<" <jobname> <input file list> <width fraction> <bin width> <option>"<<endl;
    exit(0);
  }
  ...
  double fraction=atof(argv[3]);	// Width fraction
  double binw=atof(argv[4]);		// 10 GeV bins  
  TString option=argv[argc-1];

  TString sampleName="unknown";

  if(fabs(fraction-0.10)<epsilon)      sampleName="W10p";
  else if(fabs(fraction-0.07)<epsilon) sampleName="W7p";
  else if(fabs(fraction-0.06)<epsilon) sampleName="W6p";
  else if(fabs(fraction-0.05)<epsilon) sampleName="W5p";
  else if(fabs(fraction-0.03)<epsilon) sampleName="W3p";
  else if(fabs(fraction-0.01)<epsilon) sampleName="W1p";
  else if(fraction<0)                  sampleName="NWA";
  cout<<fraction<<" "<<sampleName<<" "<<fabs(fraction-0.10)<<endl;
  ...
  const int npoint=4;
  double massArr[npoint]={400, 750, 800, 1500};
  ...
  for(int ipoint=0;ipoint<npoint;ipoint++){
    widthArr[ipoint]=fraction>0?massArr[ipoint]*fraction:0.004;
    if(fraction>0){
      xMinArr[ipoint]=massArr[ipoint]-10*widthArr[ipoint];
      xMaxArr[ipoint]=massArr[ipoint]+10*widthArr[ipoint];
    }
    else{
  ...
  for(int ipoint=0;ipoint<npoint;ipoint++){
    if(MCCat[ipoint]->sumEntries()<epsilon) continue;
    ...
    TString outputCanvasName="fig/fit/"+jobname+Form("/plot_%s_%.0f", sampleName.Data(), massArr[ipoint]);
