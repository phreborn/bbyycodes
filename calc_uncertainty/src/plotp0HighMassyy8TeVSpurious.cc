...
int main(int argc, char** argv) 
{
  ...
  pair<double, double> maxdata=readInput("pvalue/high_mass_diphoton_8TeV_nwa_updated_spurious/",data);
  double Z = maxdata.second;
  double M_minimum = maxdata.first;

  pair<double, double> maxdata2=readInput("pvalue/high_mass_diphoton_8TeV_nwa/",data2);
  ...
  g_data->SetMinimum(5e-3);
  //g_2->Draw("ae3");
  //g_1->Draw("e3");

  //  g_sm ->Draw("l");
  g_data -> SetLineColor(2);
  g_data -> SetLineStyle(7);
  ...
  leg->AddEntry(g_data,"Updated spur. signal","L");
  leg->AddEntry(g_data2,"EOYE spur. signal","L");
  ...
  PrintCanvas(c1,"fig/plot_p0/high_mass_diphoton/pvalue_new_spurious");
