...
int main(int argc, char **argv){
  ...
  for(int ievt=0;ievt<nevt;ievt++){
    ...
    xCat[icat]->setVal(p->mgg);
    wCat[icat]->setVal(p->total_weight);
    // cout<<x->getVal()<<" "<<w.getVal()<<endl; getchar();
    if(p->mgg>xMinArr[icat]&&p->mgg<xMaxArr[icat])
      MCCat[icat]->add(RooArgSet(*xCat[icat], *wCat[icat]), p->total_weight);
  }
