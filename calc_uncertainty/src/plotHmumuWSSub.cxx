...
TString categoryTranslator(TString channelname){
  if(channelname.Contains("Central_LowPtll")) return "Central low #it{p_{T}^{#mu#mu}}";
  if(channelname.Contains("Central_MidPtll")||channelname.Contains("Central_MedPtll")) return "Central medium #it{p_{T}^{#mu#mu}}";
  if(channelname.Contains("Central_HighPtll")) return "Central high #it{p_{T}^{#mu#mu}}";
  if(channelname.Contains("Forward_LowPtll")) return "Non-central low #it{p_{T}^{#mu#mu}}";
  if(channelname.Contains("Forward_MidPtll")||channelname.Contains("Forward_MedPtll")) return "Non-central medium #it{p_{T}^{#mu#mu}}";
  if(channelname.Contains("Forward_HighPtll")) return "Non-central high #it{p_{T}^{#mu#mu}}";
  if(channelname.Contains("VBF_Loose")) return "VBF loose";
  if(channelname.Contains("VBF_Tight")) return "VBF tight";
...
void addData(TH1D *hdata, vector<RooDataSet*> dataArr, vector<RooRealVar*> obsArr){
  ...
  for( int i = 0 ; i < hdata->GetNbinsX(); i ++ ) hdata->SetBinError(i+1,0);
}
...
int main( int argc, char** argv ){
  ...
  RooDataSet* m_data = (RooDataSet*)w->data("asimovData_1");
  ...
  for ( int i= 0; i < numChannels; i++ ) {
    ...
    hframe->GetYaxis()->SetTitle(Form("Events / 0.1 GeV"));
    ...
    frame->GetYaxis()->SetTitle(Form("Events / 0.1 GeV"));
    ...
    lg->SetFillColor(0);
    lg->SetShadowColor(0);
    lg->AddEntry(hdata,"Asimov dataset","EP");
    lg->AddEntry("Background","Bkg. fit","L");
    lg->AddEntry("Signal","Signal fit","L");
    lg->Draw("same");
    ...
    for( int i = 0 ; i < Nbins; i ++ ){
      ...
      // double weight = hbkg->GetBinContent(i+1)*binwidth;
    ...
    sigPdf->plotOn(frameS,LineColor(kRed),Normalization(nSig));
