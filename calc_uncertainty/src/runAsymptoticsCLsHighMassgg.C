...
//check inputs
  ...
  if(option.Contains("nwa")){
    w->var("wX")->setVal(0.004);
  }

  if(option.Contains("lwa")){
    w->var("wX")->setConstant(false);
    w->var("wX")->setRange(0,100);
  }
  if(option.Contains("changemass")){
    w->var("mX")->setVal(Hmass);
    w->var("mX")->setConstant(true);
  }

  firstPOI = (RooRealVar*)mc->GetParametersOfInterest()->first();
  if(option.Contains("gauge")) firstPOI->setRange(0,100);
...
//save the snapshots of nominal parameters, but only if they're not already saved
  ...
  if (doConditional && doFit)
  {
    ...
    // mc->GetNuisanceParameters()->Print("v"); getchar();
