...
int main(int argc, char **argv){
  ...
  unique_ptr<TFile> f(TFile::Open("workspace/high_mass_diphoton/2012_Graviton_histfactory_EKEI_v0.root","read"));
  RooWorkspace *ws=dynamic_cast<RooWorkspace*>(f->Get("combWS"));
  
  RooAbsPdf *pdf=ws->pdf("pdf_signal_inclusive_8TeV");
  ...
  for(int ievt=0;ievt<nevt;ievt++){
    ...
    if(mass>xMinArr[imass][iwidth]&&mass<xMaxArr[imass][iwidth])
  ...
  for(int imass=0;imass<nmass;imass++){
	...
	continue;
      ...
      // if(massArr[imass]==500&&widthArr[iwidth]>0.15){
      // 	cout<<"Cutting the edge. not a good physicist"<<endl;
      // 	continue;
      // }
