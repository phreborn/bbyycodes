...
int main(int argc, char** argv){
  ...
  vector<double> truthmass = { 200, 250, 300, 350, 400, 450, 500, 600, 700, 750, 800, 900, 1000, 1500, 2000 };
  ...
  string prefix = "HSG1/MxAOD/h012/mc15c/mc15c.";
  string postfix= "p2616.h012.root";
  ...
  TTree* tin = nullptr;
  for( int i=0; i<npoints; i++ ){
    fin = TFile::Open( Form("%sPowhegPy8_ggH%.0f.MxAOD.%s",prefix.c_str(),truthmass[i], postfix.c_str) );
    tin = dynamic_cast<TTree*>(fin->Get("CollectionTree"));

    MxAODNtup *p=new MxAODNtup(tin);
    ...
    for( int j=0; j<tin->GetEntries(); j++ ){
      ...
      sum_of_weight_tree += p->vertexWeight*p->mcEventWeights->at(0);
    }
    ...
    for( int j=0; j<tin->GetEntries(); j++ ){
      ...
      if( !p->isPassedLowHighMyy ) continue;
      int index1=-1, index2=-1;
      double pt1=0, pt2=0;
      for( size_t idx=0; idx<p->pt->size(); idx++ ){
	if( TMath::Abs(p->eta_s2->at(idx)) > 2.37 ) continue;
	if( p->pt->at(idx)>pt1 ){
	  pt2 = pt1; index2 = index1;
	  pt1 = p->pt->at(idx); index1 = idx;
	} else if( p->pt->at(idx)>pt2 ){
	  pt2 = p->pt->at(idx); index2 = idx;
	}
      }
      if( index1<0 || index2<0 ) continue;
      bool Barrel1 = TMath::Abs(p->eta_s2->at(index1))<=1.37;
      bool Barrel2 = TMath::Abs(p->eta_s2->at(index2))<=1.37;
      bool Unconv1 = p->conversionType->at(index1)==0;
      bool Unconv2 = p->conversionType->at(index2)==0;
      ...
      bool isEta05 = TMath::Abs(p->eta_s2->at(index1))<=0.5 && TMath::Abs(p->eta_s2->at(index2))<=0.5;
      bool isEta06 = TMath::Abs(p->eta_s2->at(index1))<=0.6 && TMath::Abs(p->eta_s2->at(index2))<=0.6;
      bool isEta07 = TMath::Abs(p->eta_s2->at(index1))<=0.7 && TMath::Abs(p->eta_s2->at(index2))<=0.7;
      bool isEta08 = TMath::Abs(p->eta_s2->at(index1))<=0.8 && TMath::Abs(p->eta_s2->at(index2))<=0.8;
      bool isEta10 = TMath::Abs(p->eta_s2->at(index1))<=1.0 && TMath::Abs(p->eta_s2->at(index2))<=1.0;
      bool isEta12 = TMath::Abs(p->eta_s2->at(index1))<=1.2 && TMath::Abs(p->eta_s2->at(index2))<=1.2;

      bool isEPSC1 = isUU && TMath::Abs(p->eta_s2->at(index1))<=0.75 && TMath::Abs(p->eta_s2->at(index2))<=0.75;
      bool isEPSC2 = isUU && !(TMath::Abs(p->eta_s2->at(index1))<=0.75 && TMath::Abs(p->eta_s2->at(index2))<=0.75);
      bool isEPSC3 = !isUU && TMath::Abs(p->eta_s2->at(index1))<=0.75 && TMath::Abs(p->eta_s2->at(index2))<=0.75;
      bool isEPSC4 = isUU && !(TMath::Abs(p->eta_s2->at(index1))<=0.75 && TMath::Abs(p->eta_s2->at(index2))<=0.75) && (TMath::Abs(p->eta_s2->at(index1))<=1.3 || TMath::Abs(p->eta_s2->at(index1))>=1.75) && (TMath::Abs(p->eta_s2->at(index2))<=1.3 || TMath::Abs(p->eta_s2->at(index2))>=1.75);
      ...
      bool isTran = (TMath::Abs(p->eta_s2->at(index1))>1.30&&TMath::Abs(p->eta_s2->at(index1))<1.75) || (TMath::Abs(p->eta_s2->at(index2))>1.30&&TMath::Abs(p->eta_s2->at(index2))<1.75);
